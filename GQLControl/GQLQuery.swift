...
open class GQLQuery2<Value, QueryType: GraphQLOperation>: Query<Value>, _GQLQuery where QueryType.Data: GQLData {
    ...
    func setupExecution() {
        self.execution = {[unowned self] (completion) in
            
            let cachePolicy = self.cachePolicy
            let apolloOperation = self.apolloOperation
            let apolloQueue = self.apolloQueue
            let decoder = self.decoder
            let didLoadData = self.didLoadData
            
            self.cancellable = apolloOperation.execute(on: apolloQueue, cachePolicy: cachePolicy) { (result, error) in
